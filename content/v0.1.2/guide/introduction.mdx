...
| **安全校验** | 外部 `htmlBody` 注册即校验（fail-closed，S2）：拒绝内联脚本 / `on*` 事件 / 外链 URL / `oreui://` 越界 | v0.1.2 起启用 |
...
// IDearOreUIApi* oreui = ...; // 经 C ABI 桥 DearOreUI_QueryApi 获取，见 environment 页
...
dearoreui::api::ComponentSpec text;
text.kind    = dearoreui::api::ComponentKind::Text;
...
`ComponentSpec` 树经过 `ComponentRenderer` 变成 `DomNode` 树，再走通用的 CSSOM 渲染器，和普通 Overlay 是同一条管线。组件怎么拼、字段代表什么，见[组件总览](../components/overview)。
...
- **能力矩阵**：`CapabilitySet` 公开 21 项能力的支持等级（`Unknown/Unsupported/Experimental/Supported`）。运行时支持到什么程度，查一下就知道。
...
- **显式错误**：`Result<T>` / `ErrorCode` 贯穿所有公共 API，集成代码可以显式处理冲突、校验失败、无效参数。
...
阶段 8       已交付（2026-08-22 核对）
   ...
   JS→C++ Facet 通道（单发限制）/ API 门面（Runtime/Resource/Mod/UI/Host 五组接口）
```
...
这两条已经做到；v1.0 候选收敛还在路上，见[路线图](./roadmap)。
...
| 外部 API 入口 | 当前参考内置 Demo 的 `runtime::IRuntime::api()`，稳定获取方式见[在你的模组中使用](./environment) |
...
- [安装与构建](./installation) —— 下载发布版或从源码构建
- [组件总览](../components/overview) —— 37 种组件的用法
- [在你的模组中使用](./environment) —— 完整集成指南