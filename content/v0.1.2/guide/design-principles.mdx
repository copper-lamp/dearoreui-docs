...
零侵入原则的延伸是**你不该把不可信内容直接塞进页面**。自 v0.1.2 起，所有外部 `htmlBody` 在注册时即经过 fail-closed 安全校验：拒绝 `<script>` / `<iframe>` / `<object>`、`on*` 事件属性、`javascript:` / `file:` / `http:` / `data:` URL，以及越界的 `oreui://` 路径。校验不通过会**直接拒绝注册**，而不是带病放行。

动态交互请交给页面脚本 + `oreui.event.on`，不要依赖内联脚本。
