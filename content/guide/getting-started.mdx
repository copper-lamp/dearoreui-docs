...
示例里的 `oreui` 入口当前取自内置 Demo 的注册方式：`runtime::IRuntime::api()` 返回 `IDearOreUIApi*`，再调用 `registerMod` → `registerOverlay`，流程与本例一致。完整的集成步骤见[在你的模组中使用](./environment)。
...
- 了解 [声明依赖与 Manifest](./environment#第一步声明依赖)（资源、脚本、样式的声明方式）
- 了解 [UI 类型与生命周期](./environment#ui-类型速查)（Overlay、Panel、Button 的区别）
- 了解 [诊断与排查](./environment#诊断文件)（诊断事件字段、能力矩阵）
